...
const EntryForm = (props) => {
  ...
  const getPermissions = async () => {
    ...
    const filtered = resources
      .filter((x) => !result.data.some((y) => y.resourceId === x._id))
      .map((z) => ({
        resourceAlias: z.alias,
        resourceId: z._id,
        resourceName: z.name,
        isAllowed: false,
        isDisabled: false,
  ...
  const columns = [
    {
      ...
      tip: 'Resource name',
      render: (dom, entity) => {
        return <>{`${entity.resourceAlias} (${entity.resourceName})`}</>;
      },
    },
    {
      title: 'Role',
  ...
  return (
    <PageContainer pageHeaderRender={false}>
      <Card className="Permission Manage Form" bordered={false}>
        <ProFormSelect
          width="lg"
          name="roleId"
          label="Roles"
