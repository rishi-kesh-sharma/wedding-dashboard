...
import { saveLoveStory, updateLoveStory } from '@/pages/event/service';

const LoveStoryModal = (props) => {
  ...
  const callApi = async (values) => {
    ...
    if (current) {
      const result = await updateLoveStory(current._id, formData);
      ...
      const result = await saveLoveStory(eventId, formData);
      if (result instanceof Error || result.status == 'error' || result.success == false) {
        message.error(result.message);
      } else {
